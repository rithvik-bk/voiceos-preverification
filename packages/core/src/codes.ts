...
export const REASON_CODES: Record<string, ReasonCode> = {
  provenance_mismatch: {
    ...
    routing: true,
    summary:
      'a Tier-3 destination/amount/permission slot resolved to a referent that carries NO ' +
  ...
  insufficient_provenance: {
    ...
    routing: true,
    summary:
      'a routing param resolved, but its grounding source ranks below the routing minimum ' +
  ...
  amount_not_in_speech: {
    ...
    routing: true,
    summary:
      'the filled amount is not in the spoken number-set (number-twin / magnitude); ' +
  ...
  misbound_param: {
    ...
    routing: true,
    summary:
      'each routing param was spoken, but they trace to DIFFERENT transcript clauses — the ' +
  ...
  ambiguous_target: {
    ...
    routing: true,
    summary:
      'two or more real targets match the spoken descriptor (|candidates| ≥ 2); selection ' +
  ...
  target_not_found: {
    ...
    routing: true,
    summary:
      'a routing param resolved to no grounded referent (hallucinated / misheard target). ' +
