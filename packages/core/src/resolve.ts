...
export interface Target {
  id: string; // machine id, adapter-defined shape
  label: string; // '#eng-backend' | 'Priya Sharma' — the ONLY string a user surface may show
  kind: EntityKind;
...
export interface CandidateSummary {
  name: string; // human, card-safe
  kind: EntityKind;
  id?: string; // machine field, never rendered
}
...
export function resolveTarget(spoken: string, pool: Target[]): Resolution {
  ...
  if (known !== undefined) return { match: known, candidates: [known], reason: 'match' };
