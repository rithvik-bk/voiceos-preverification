...
| **LLM-as-judge / a verifier model** | A second AI's opinion on the first AI's output | This is AI grading AI — the exact objection any rigorous engineer raises. It adds latency, it adds a *second* thing that can hallucinate, and the two models share failure modes, so they tend to be wrong about the same inputs at the same time. |
